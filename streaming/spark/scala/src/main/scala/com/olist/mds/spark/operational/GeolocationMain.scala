...
object GeolocationMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (spark.catalog.tableExists(TargetTable)) {
      val existingCount = spark.table(TargetTable).count()
      if (existingCount > 0) {
        ...
        println(
          s"Geolocation table $TargetTable already populated with $existingCount rows. No-op."
        )
        return
      }
    }

    projectedDf.writeTo(TargetTable).append()
    val finalCount = spark.table(TargetTable).count()
    println(s"Geolocation dataset written to $TargetTable cleanly with $finalCount rows.")
  }
}
