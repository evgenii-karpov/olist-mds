...
class StageVOracleTests(unittest.TestCase):
    def setUp(self) -> None:
        self.assertTrue(
            ORACLE_PATH.exists(), f"Oracle JSON file missing: {ORACLE_PATH}"
        )
        self.oracle_data = json.loads(ORACLE_PATH.read_text(encoding="utf-8"))

    def test_initial_snapshot_counts(self) -> None:
        init = self.oracle_data["initial_snapshot"]
    ...
    def test_crud_delta_breakdown(self) -> None:
        crud = self.oracle_data["crud_delta"]
    ...
    def test_post_crud_totals(self) -> None:
        post_crud = self.oracle_data["post_crud"]
    ...
    def test_post_schema_totals(self) -> None:
        post_schema = self.oracle_data["post_schema"]
