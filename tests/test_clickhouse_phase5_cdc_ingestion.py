...
class ClickHousePhase5CdcIngestionTests(unittest.TestCase):
    ...
    def test_local_lab_helper_uses_current_bootstrap_contract(self) -> None:
        ...
        self.assertIn('"run-workload"', helper)
        self.assertIn('"scripts.simulation"', helper)
        self.assertIn('"run"', helper)
        self.assertIn('"--event-limit"', helper)
        self.assertIn('"--duration-seconds"', helper)
